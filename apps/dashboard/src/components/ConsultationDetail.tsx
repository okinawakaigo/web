...
export default function ConsultationDetail({ id, onChange, onClose, onEditorChange }: DetailProps) {
  ...
  const content = <>
    <header className="detail-header detail-header--compact">
      <div>
        <p className="detail-eyebrow">説明会への参加相談 / 相談者への返信</p>
        <div className="detail-heading-main"><h2 id="detail-title" ref={heading} tabIndex={-1}>{item?.name ?? '相談の詳細'}</h2>
          {item && <><StatusBadge status={item.status} /><span className="detail-heading-email" title={item.email}>{item.email}</span></>}
        </div>
      </div>
      <div className="detail-header-actions">
        {item && !loading && <button className="icon-button" aria-label="返信履歴を更新" title="返信履歴を更新" onClick={() => setReplyRefresh(value => value + 1)} disabled={processing}><Icon name="refresh" /></button>}
    ...
    {item && !loading && <ReplyThread key={id} item={item} refresh={replyRefresh} disabled={busy} onEditorChange={reportReplyEditor} />}
    ...
    <div className="detail-body">
      ...
      {item && !loading && <>
        <section className="detail-section" aria-labelledby="contact-title"><h3 id="contact-title">連絡先</h3><p className="contact-email">{item.email}</p><p className="subtle-text"><time dateTime={item.createdAt}>{formatDate(item.createdAt)} 受付</time></p></section>
