...
interface Props { item: Consultation; refresh?: number; disabled: boolean; onEditorChange(dirty: boolean, busy: boolean): void }
...
export default function ReplyThread({ item, refresh = 0, disabled, onEditorChange }: Props) {
  ...
  return <section className="reply-workspace" aria-label="相談者への返信">
    <div className="reply-history" aria-label="相談と送信履歴" aria-busy={loading}>
      {historyError && <div className="reply-error" role="alert"><p>{historyError}</p><Button compact variant="outline" disabled={loading || busy} onClick={() => void loadHistory()}>履歴を更新</Button></div>}
    ...
    <form className="reply-composer" onSubmit={event => void send(event)} aria-label="メール返信">
      {history?.settings.localTest ? <p className="reply-test-notice">ローカル確認用です。返信を記録しますが、メールは送信しません。</p>
        : history && unavailable ? <p className="reply-test-notice">メール返信の設定が完了していません。</p>
          : history && <details className="reply-delivery"><summary>送信元・返信先</summary><p className="reply-routing">送信元：{history.settings.from}<br />相談者からの返信は {history.settings.replyTo} で受信します。</p><p className="reply-footnote">送信受付済みのメールの配信結果はResendで確認できます。</p></details>}
      <div className="reply-subject-field"><TextField compact id="reply-subject" label="件名" required maxLength={replyLimits.subject} value={subject} onChange={event => setSubject(event.target.value)} disabled={busy || loading || disabled || unavailable} readOnly={!!attempt} /></div>
      <div className="reply-body-field"><TextArea id="reply-body" label="返信本文" required rows={2} maxLength={replyLimits.body} placeholder="返信を入力してください。" value={body} onChange={event => setBody(event.target.value)} disabled={busy || loading || disabled || unavailable} readOnly={!!attempt} /></div>
